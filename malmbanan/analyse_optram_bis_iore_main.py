...
# Paths
home = os.getcwd()
data_directory = 'C:/Users/AbdouAA/Desktop/Mistra-infra_maint/data_malmbanan/section111/'
O2_path = os.path.abspath(os.path.join(os.path.dirname(home),data_directory+'OPTRAM'))
O1_path = os.path.abspath(os.path.join(os.path.dirname(home),data_directory+'OPTRAM/alignment'))
B_path = os.path.abspath(os.path.join(os.path.dirname(home),data_directory+'BIS'))
S_path = os.path.abspath(os.path.join(os.path.dirname(home),data_directory+'SHAPE'))
IORE_path = os.path.abspath(os.path.join(os.path.dirname(home),data_directory+'LOCO'))
...
O1_filename = ['cirkulärkurva.xlsx','raklinje.xlsx','rälsförhöjning.xlsx','sth.xlsx','räl.xlsx','övergångskurva.xlsx']
O3_filename = 'curves_SPL_1909250857A_111_E_1417_874_20210223_1107.xlsx'
O2_filename = ['RPM_1409121027A_111_E_1417_876_20210223_1105.csv', #'RPM_1409121027A_111_E_1417_876_20210223_1106.csv' (skipped cz same as 1105),
...
# Columns to select from the files
B_columns = np.array([[2,3,4,7,8,9]]) # from tamping & grinding excel file
...
val_s, label_s = open_xls(S_path,S_filename,S_columns) # coordinates of curves
val_b, label_b = open_xls(B_path,B_filename,B_columns) # tamping data
...
nr_cat_100,L_cat_100,cat_ryear_100,cat_veq_100,cat_prof_100,cat_maint_100,cat_gauge_100,cat_H_100,cat_i_100, cat_L_100 = cat_curve_opt(O_func,maint,R_cat_100,damage)
...
cat_year_sort_100,cat_year_label_100,cat_veq_sort_100,cat_veq_label_100,cat_prof_sort_100,\
cat_prof_label_100,cat_maint_sort_100,cat_maint_label_100,cat_gauge_sort_100,cat_gauge_label_100,\
    cat_H_sort_100,cat_H_label_100=sort_R_cat(cat_ryear_100,cat_veq_100,cat_prof_100,cat_maint_100,cat_gauge_100,nr_cat_100,cat_H_100)


cat_year_sort_100_L,cat_veq_sort_100_L,cat_prof_sort_100_L,cat_maint_sort_100_L,cat_gauge_sort_100_L,cat_H_sort_100_L\
    =sort_R_cat_L(cat_ryear_100,cat_veq_100,cat_prof_100,cat_maint_100,cat_gauge_100,nr_cat_100,cat_H_100,cat_L_100)
...
if Y == 'nr':
    if C == '':
        ...
        plt.figure(1,figsize=(14,6)) #passage 1
        ax = plt.axes()
        ax.bar(R_cat_100, cat_H_sort_100[:,0], width = 90,color = 'b', label = r'$\Delta_H$' ' less than 0.33 mm/year')
        ax.bar(R_cat_100, cat_H_sort_100[:,1], bottom = cat_H_sort_100[:,0],width = 90,color = 'r', label = r'$\Delta_H$' ' between 0.33 - 0.66 mm/year')
        ax.bar(R_cat_100, cat_H_sort_100[:,2], bottom = cat_H_sort_100[:,0]+cat_H_sort_100[:,1],width = 90,color = 'g', label =  r'$\Delta_H$' ' between 0.66 - 1 mm/year')
        ax.bar(R_cat_100, cat_H_sort_100[:,3], bottom = cat_H_sort_100[:,0]+cat_H_sort_100[:,1]+cat_H_sort_100[:,2],width = 90,color = 'm', label = r'$\Delta_H$' ' larger than 1 mm/year')
        plt.xticks(np.arange(100,3100,100)-100/2, np.arange(0,3000,100),rotation=45) #('','','','',500,'','','','',1000,'','','','',1500,'','','','',2000,'','','','',2500,'','','','',3000)
        plt.xticks(fontsize=22)
        plt.yticks(fontsize=22)
        plt.ylabel('Number of curves [-]',fontsize=24,labelpad=10)
