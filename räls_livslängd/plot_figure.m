...
elseif strcmp(wanted_fig, 'LCC_heatmap')
    ...
    if strcmp(arg{2}, 'contours')
        ...
        ylabel('Grinding interval (in months)')
        xlabel('Gauge correction interval (in months)')
        zlabel('Annuity per meter (in SEK/year)')
...
elseif strcmp(wanted_fig, 'historic')
    ...
    ylabel('RCF residual (mm)');
    legend({'H-index (left values)', 'RCF residual (right values)'}, 'Location', 'best');
    xlabel('Rail age (months)');
    ...
    ylim([1440, 1455]);
    xlim([1, rail_lifetime_months]);
    ylabel('Track gauge (mm)');
    xlabel('Rail age (months)');
    grid on;
    legend({'Track gauge'}, 'Location', 'best');
...
elseif strcmp(wanted_fig, 'interpolation')
    ...
    surf(X, Y, H_table);
    colorbar;
    xlabel('Months (since last grinding)', 'Rotation', 17);
    ylabel('Track gauge (mm)', 'Rotation', -30);
    zlabel(strcat(arg{2}, ' (mm)'));
...
function plotSensitivityAnalysisGaugeWidening(arg)
    ...
    ylabel('Annuity per meter (SEK per year)');
    xlabel('Initial gauge widening (mm/y)');
    xticks(gauge_widening_values);
    grid on;
    legend({'Lifetime', 'Annuity'}, 'Location', 'best');
    ...
    ylabel('Optimal gauge correction interval (months)');
    xlabel('Initial gauge widening (mm/y)');
    xticks(gauge_widening_values);
    grid on;
    legend({'Rail grinding', 'Gauge correction'}, 'Location', 'best');
end
...
function plotSensitivityAnalysisRenewalCosts(arg)
    ...
    xlabel('Renewal cost factor (percent)');
    grid on;
    legend({'Lifetime', 'Annuity'}, 'Location', 'best');
...
function plotSensitivityAnalysisMaintenanceCosts(arg)
    ...
    ylabel('Annuity per meter (SEK per year)');
    xlabel('Maintenance cost factor (percent)');
    grid on;
    legend({'Lifetime', 'Annuity'}, 'Location', 'best');
...
function plotSensitivityAnalysisRenewalCostsGaugeWidening(arg)
    ...
    for i = 1:length(gauge_widening_values)
        ...
        grid on;
        legend({'Optimal Lifetime', 'ANN'}, 'Location', 'best');
