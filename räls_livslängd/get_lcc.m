...
for m=1:180 % till track lifetime
    ...
    lifetime_remainder = min([lifetime_remainder max_lifetime(row)]);
...
end_of_life = -get_renewal_cost(track_lifetime_years, inner_rail)*(2*rail_lifetime-track_lifetime_years)/rail_lifetime;


% total LCC in NPV/meter
lcc_total = train_op/track_length_meter + (maintenance+renewal+end_of_life)/track_length_meter;

if(rail_lifetime==180)
    return;
end
