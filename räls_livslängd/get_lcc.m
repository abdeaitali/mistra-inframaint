...
for m=1:max_m % till track lifetime
    ...
    %delta_H = interp2(Yq,Xq, NW_table(:,Yq),latest_grinding_since,gauge_curr,'linear');
    ...
    if(latest_grinding_since==grinding_freq) % time to do grinding
        ...
        %H_curr = H_curr + interp2(Yq,Xq, H_table(:,Yq),grinding_freq,gauge_curr,'linear') - delta_H;
        H_curr = H_curr + pchip(Xq, H_table(:, grinding_freq), gauge_curr) - delta_H;
        if(isnan(H_curr))
            H_curr
        end
        % Update the RCF residual
        % rcf_grinding = interp2(Yq,Xq, RCF_residual_table(:,Yq),grinding_freq,gauge_curr,'linear');
        ...
        %%% update the RCF residual
        % RCF_residual_curr = RCF_res_grinding + interp2(Yq,Xq, RCF_depth_table(:,Yq),latest_grinding_since,,type);
    ...
    if(RCF_residual_curr>RCF_max)
        ...
        % update the H-index
        %delta_H_1 = interp2(Yq,Xq, H_table(:,Yq),latest_grinding_since+1,gauge_curr,type);%-delta_H;
        %delta_H_2 = interp2(Yq,Xq, H_table(:,Yq),1,gauge_curr,type);
        delta_H_1 = pchip(Xq, H_table(:, latest_grinding_since+1), gauge_curr);
        delta_H_2 = pchip(Xq, H_table(:, 1), gauge_curr);
        if(isnan(delta_H_1) || isnan(delta_H_2))
            delta_H_1
            delta_H_2
        end
